...
const { delay, radioRandom } = require('../utils')
const max = 500,
  min = 200
let browser
let page
...
function getDocument(page, searchContent) {
  ...
  return new Promise(resolve => {
    setTimeout(async () => {
      ...
      // 随机进行radio选择
      await page.click(`#j-type > label:nth-child(12) > input`, { delay: delay(max, min) })
      // 点击确定按钮
...
// 选择adapter
async function getAdapter(searchContent) {
  // 查找适配器
  const Adapter = require(`./adapters/${config.adapter}.js`)
  adapter = new Adapter(searchContent, config.origin)
}

module.exports = {
  // 根据搜索内容查找可下载音乐列表或者返回搜索失败或空结果
  search: function(searchContent) {
    getAdapter(searchContent)
    // 根据内容搜索，返回列表
